#!/bin/bash
# MNEMION SAVE HOOK — Auto-save every N exchanges
#
...
# 3. Returns a reason telling the AI to save structured diary + Anaktoron entries
# 4. AI does the save (topics, decisions, code, quotes → organized into the Anaktoron)
...
#       "hooks": [{
#         "type": "command",
#         "command": "/absolute/path/to/mnemion_save_hook.sh",
#         "timeout": 30
#       }]
#     }]
#   }
...
#
# === MNEMION CLI ===
# This repo uses: mnemion mine <dir>
# or:            mnemion mine <dir> --mode convos
# Set MNEMION_DIR below if you want the hook to auto-ingest after blocking.
# Leave blank to rely on the AI's own save instructions.
#
# === CONFIGURATION ===

SAVE_INTERVAL=15  # Save every N human messages (adjust to taste)
STATE_DIR="$HOME/.mnemion/hook_state"
mkdir -p "$STATE_DIR"

# Optional: set to the directory you want auto-ingested on each save trigger.
# Example: MNEMION_DIR="$HOME/conversations"
...
# Log for debugging (check ~/.mnemion/hook_state/hook.log)
...
if [ "$SINCE_LAST" -ge "$SAVE_INTERVAL" ] && [ "$EXCHANGE_COUNT" -gt 0 ]; then
    ...
    # Optional: run mnemion ingest in background if MNEMION_DIR is set
    if [ -n "$MNEMION_DIR" ] && [ -d "$MNEMION_DIR" ]; then
        SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
        REPO_DIR="$(dirname "$SCRIPT_DIR")"
        python3 -m mnemion mine "$MNEMION_DIR" >> "$STATE_DIR/hook.log" 2>&1 &
    fi
