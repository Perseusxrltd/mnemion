...
Used by mnemion init before mining begins.
...
STOPWORDS = {
    ...
    "training",
    "inference",
    # Status / state adjectives common in project docs
    "current",
    "verified",
    "pending",
    "active",
    ...
    "success",
    "done",
    # Generic technical / business nouns that capitalise at sentence start
    "auto",
    "stage",
    "trust",
    "hybrid",
    ...
    "contract",
    "interface",
    # Adjective-nouns commonly capitalised mid-sentence in technical writing
    "lexical",
    "semantic",
    "syntactic",
    "logical",
...
def extract_candidates(text: str) -> dict:
    ...
    # Filter: must appear at least 5 times to be a candidate (reduces noise from
    # sentence-start capitalisation in short docs).
    return {name: count for name, count in counts.items() if count >= 5}
...
def detect_entities(file_paths: list, max_files: int = 10) -> dict:
    ...
    # Only surface uncertain entities that appear frequently enough to be worth a
    # human look — confidence 0.3 means the word appeared 15+ times.
    uncertain = [e for e in uncertain if e["confidence"] >= 0.3]
...
def confirm_entities(detected: dict, yes: bool = False) -> dict:
    ...
    """
    print(f"\n{'=' * 58}")
    print("  Mnemion — Entity Detection")
    print(f"{'=' * 58}")
