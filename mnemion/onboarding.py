#!/usr/bin/env python3
"""
onboarding.py — Mnemion first-run setup.

Asks the user:
  1. How they're using Mnemion (work / personal / combo)
...
Seeds the entity_registry with confirmed data so Mnemion knows your world
from minute one — before a single session is indexed.

Usage:
    python3 -m mnemion.onboarding
    or: mnemion init
"""

from pathlib import Path
from mnemion.entity_registry import EntityRegistry
from mnemion.entity_detector import detect_entities, scan_for_detection
...
def _ask_mode() -> str:
    _header("Welcome to Mnemion")
    print("""
  Mnemion is a personal memory system. To work well, it needs to know
...
""")
    print("  How are you using Mnemion?")
    print()
...
def _ask_projects(mode: str) -> list:
    ...
    _hr()
    print("""
  What are your main projects? (These help Mnemion distinguish project
...
def _warn_ambiguous(people: list) -> list:
    ...
    """
    from mnemion.entity_registry import COMMON_ENGLISH_WORDS
...
def _generate_aaak_bootstrap(
    ...
    """
    mnemion_dir = Path(config_dir) if config_dir else Path.home() / ".mnemion"
    mnemion_dir.mkdir(parents=True, exist_ok=True)
    ...
    registry_lines = [
        "# AAAK Entity Registry",
        "# Auto-generated by mnemion init. Update as needed.",
        "",
        "## People",
    ]
    for p in people:
    ...
    (mnemion_dir / "critical_facts.md").write_text("\n".join(facts_lines))
...
def run_onboarding(
  ...
  Mnemion will check the context before treating them as person names.
...
""")
    ...
    print("\n  AAAK entity registry: ~/.mnemion/aaak_entities.md")
    print("  Critical facts bootstrap: ~/.mnemion/critical_facts.md")
