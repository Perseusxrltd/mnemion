"""
Hook logic for Mnemion — Python implementation of session-start, stop, and precompact hooks.
...
SAVE_INTERVAL = 15
STATE_DIR = Path.home() / ".mnemion" / "hook_state"
...
def _maybe_auto_ingest():
    """If MNEMION_DIR is set and exists, run mnemion mine in background."""
    mnemion_dir = os.environ.get("MNEMION_DIR", "")
    if mnemion_dir and os.path.isdir(mnemion_dir):
        try:
            log_path = STATE_DIR / "hook.log"
            with open(log_path, "a") as log_f:
                subprocess.Popen(
                    [sys.executable, "-m", "mnemion", "mine", mnemion_dir],
                    stdout=log_f,
                    stderr=log_f,
                )
        except OSError:
...
def hook_stop(data: dict, harness: str):
    ...
    if since_last >= SAVE_INTERVAL and exchange_count > 0:
        ...
        # Optional: auto-ingest if MNEMION_DIR is set
        _maybe_auto_ingest()
