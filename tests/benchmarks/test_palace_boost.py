"""
Anaktoron boost validation — does wing/room filtering actually help?

Quantifies the retrieval improvement from the Anaktoron spatial metaphor.
...
class TestFilteredVsUnfilteredRecall:
    """Quantify Anaktoron boost: recall improvement from wing/room filtering."""

    SIZES = [1_000, 2_500, 5_000]

    @pytest.mark.parametrize("n_drawers", SIZES)
    def test_anaktoron_boost_recall(self, n_drawers, tmp_path, bench_scale):
        ...
        _, _, needle_info = gen.populate_anaktoron_directly(
            anaktoron_path, n_drawers=n_drawers, include_needles=True
        )
        ...
        for needle in needle_info[:n_queries]:
            ...
            result = search_memories(
                needle["query"], anaktoron_path=anaktoron_path, wing=needle["wing"], n_results=5
            )
            texts = [h["text"] for h in result.get("results", [])]
            if any("NEEDLE_" in t for t in texts[:5]):
...
class TestFilterLatencyBenefit:
    ...
    def test_filter_speedup(self, tmp_path, bench_scale):
        ...
        gen = AnaktoronDataGenerator(seed=42, scale=bench_scale)
        anaktoron_path = str(tmp_path / "anaktoron")
        gen.populate_anaktoron_directly(anaktoron_path, n_drawers=5_000, include_needles=False)
        ...
        for _ in range(n_runs):
            ...
            search_memories(query, anaktoron_path=anaktoron_path, n_results=5)
        ...
        for _ in range(n_runs):
            start = time.perf_counter()
            search_memories(query, anaktoron_path=anaktoron_path, wing=wing, n_results=5)
        ...
        for _ in range(n_runs):
            start = time.perf_counter()
            search_memories(query, anaktoron_path=anaktoron_path, wing=wing, room=room, n_results=5)
...
class TestBoostAtIncreasingScale:
    """Does the Anaktoron boost increase as the Anaktoron grows?"""
